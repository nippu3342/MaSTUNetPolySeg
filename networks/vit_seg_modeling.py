...
from . import vit_seg_configs_original as configs
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, skip= None): # It doesn't matter what you set here for skip as it is controlled by external function call
        x = self.up(x)
        if skip is not None:
            x = torch.cat([x, skip], dim=1)
            #print('skip is not none')
        #print('The shape of x = ', x.shape)
        x = self.conv1(x)
        #print('The shape of x after conv 1 = ', x.shape)
        x = self.conv2(x)
...
class DecoderCup(nn.Module):
    def __init__(self, config):
        ...
        in_channels = [head_channels] + list(decoder_channels[:-1]) #This line of code creates a list called in_channels by concatenating head_channels with a subset of the decoder_channels list.
    ...
    def forward(self, hidden_states, features = None): # It doesn't matter what you set here for features as it is controlled by external function call
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=256, num_classes=1, zero_head=False, vis=False): #Ashwini changed num_classes from 21843 and img_size from 224
